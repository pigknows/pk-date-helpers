...
describe('Detects shortcuts for formats', () => {
  describe('AMERICAN shortcuts', () => {
    const conflictingFormats = ['EUROPEAN', 'REGULAR', 'THOUSAND'];
    test('4 digit', () => {
      expect(detectFormatType('0101', true, conflictingFormats)).toEqual('AMERICAN');
    });

    test('6 digit', () => {
      expect(detectFormatType('010118', true, conflictingFormats)).toEqual('AMERICAN');
    });
  });

  describe('EUROPEAN shortcuts', () => {
    const conflictingFormats = ['AMERICAN', 'REGULAR', 'THOUSAND'];
    test('4 digit', () => {
      expect(detectFormatType('0101', true, conflictingFormats)).toEqual('EUROPEAN');
    });

    test('6 digit', () => {
      expect(detectFormatType('010118', true, conflictingFormats)).toEqual('EUROPEAN');
    });
  });

  describe('JULIAN shortcuts', () => {
    const conflictingFormats = ['AMERICAN', 'EUROPEAN', 'NEWSHAM', 'REGULAR', 'THOUSAND']; // julian has a lower preference for most of its shortcuts
    test('1 digit', () => {
      expect(detectFormatType('1', true, conflictingFormats)).toEqual('JULIAN');
    });

    test('2 digit', () => {
      expect(detectFormatType('01', true, conflictingFormats)).toEqual('JULIAN');
    });

    test('3 digit', () => {
      expect(detectFormatType('001', true, conflictingFormats)).toEqual('JULIAN');
    });

    test('4 digit', () => {
      expect(detectFormatType('0101', true, conflictingFormats)).toEqual('JULIAN');
    });

    test('5 digit', () => {
      expect(detectFormatType('18001', true, conflictingFormats)).toEqual('JULIAN');
    });

    test('6 digit', () => {
      expect(detectFormatType('010118', true, conflictingFormats)).toEqual('JULIAN');
    });

    test('8 digit', () => {
      expect(detectFormatType('01012018', true, conflictingFormats)).toEqual('JULIAN');
    });
  });

  describe('NEWSHAM shortcuts', () => {
    const conflictingFormats = ['AMERICAN', 'EUROPEAN', 'JULIAN', 'REGULAR', 'THOUSAND'];
    test('4 digit', () => {
      expect(detectFormatType('0101', true, conflictingFormats)).toEqual('NEWSHAM');
    });

    test('6 digit', () => {
      expect(detectFormatType('010118', true, conflictingFormats)).toEqual('NEWSHAM');
    });
  });

  describe('REGULAR shortcuts', () => {
    // because REGULAR has the highest preference, no ignoreFormats needed
    test('4 digit', () => {
      expect(detectFormatType('0101')).toEqual('REGULAR');
    });

    test('6 digit', () => {
      expect(detectFormatType('010118')).toEqual('REGULAR');
    });
  });

  describe('THOUSAND shortcuts', () => {
    const conflictingFormats = ['REGULAR']; // the only higher preference than THOUSAND
    test('1 digit', () => {
      expect(detectFormatType('1', true, conflictingFormats)).toEqual('THOUSAND');
    });

    test('2 digit', () => {
      expect(detectFormatType('01', true, conflictingFormats)).toEqual('THOUSAND');
    });

    test('3 digit', () => {
      expect(detectFormatType('001', true, conflictingFormats)).toEqual('THOUSAND');
    });


    test('4 digit', () => {
      expect(detectFormatType('0101', true, conflictingFormats)).toEqual('THOUSAND');
    });

    test('6 digit', () => {
      expect(detectFormatType('010118', true, conflictingFormats)).toEqual('THOUSAND');
    });
  });
});
